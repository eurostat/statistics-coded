...
# https://ec.europa.eu/eurostat/statistics-explained/index.php/Interaction_of_household_income,_consumption_and_wealth_-_methodological_issues
...
matJointDist <- acast(data = jointDist, q_inc~q_exp, value.var = "values")/1000

hist3D(1:10,1:10, matJointDist, xlab = "Income", ylab = "Consumption", 
       border = "grey", space = 0.2)

setwd(path_results)
write.xlsx(matJointDist,"DataFiguresSE2.xlsx",sheetName = "Figure 1",append = TRUE)
...
setwd(path_results)
write.xlsx(figure2,"DataFiguresSE2.xlsx",sheetName = "Figure 2",append = TRUE)
...
sr_agg <- filter(indicators_sr,
                 age == "TOTAL" & time == year)
sr_na_2015 <- filter(sr_na,
                     time == year & sector == "S14_S15" & na_item == "SRG_S14_S15")

sr_na_2015 <- mutate(sr_na_2015,
                     sr_na = values)
sr_agg <- mutate(sr_agg,
                 sr_agg = 100 - values)
sr <- merge(sr_agg, sr_na_2015, by = "geo")

plot(sr$sr_agg, sr$sr_na, type = "p", pch = 18, col = col1, bty = "n", xaxt="n",yaxt="n", 
     xlim = c(0,35), ylim = c(-15,25), xlab = NA, ylab = NA)
grid(nx = NA, ny = NULL)
axis(1,pos=0)
...
setwd(path_results)
write.xlsx(sr,"DataFiguresSE2.xlsx",sheetName = "Figure 3",append = TRUE)
